...
import (
	...
	customModels "demo-tdd-vinlab.com/m/models"
...
func New(config *Config) *http.ServeMux {
	...
	mux.Handle("/github/login", customModels.StateHandler(stateConfig, customModels.LoginHandler(oauth2Config, nil)))
	mux.Handle("/github/callback", customModels.StateHandler(stateConfig, customModels.CallbackHandler(oauth2Config, issueSession(), nil)))
	return mux
}
...
func issueSession() http.Handler {
	fn := func(w http.ResponseWriter, req *http.Request) {
		ctx := req.Context()
		githubUser, err := customModels.RetrieveUserInfoGitHub(ctx)
		if err != nil {
